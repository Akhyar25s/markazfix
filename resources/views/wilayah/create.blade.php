...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-foreground">Tambah Wilayah</h1>
            <p class="text-muted-foreground mt-1 text-sm">Tambahkan data wilayah baru ke dalam sistem.</p>
        </div>
    </div>
    ...
    <x-card class="backdrop-blur-md bg-card/80 border-primary/10 shadow-xl">
        <form action="{{ route('wilayah.store') }}" method="POST" class="space-y-6">
            @csrf

            <div class="space-y-2">
                <x-label for="nama_wilayah">Nama Wilayah <span class="text-red-500">*</span></x-label>
                <x-input id="nama_wilayah" name="nama_wilayah" type="text" placeholder="Masukkan nama wilayah (contoh: Wilayah 1)" value="{{ old('nama_wilayah') }}" required />
                @error('nama_wilayah')
            ...
            <div class="space-y-2">
                ...
                <x-textarea id="deskripsi" name="deskripsi" rows="3" placeholder="Masukkan deskripsi wilayah (opsional)">{{ old('deskripsi') }}</x-textarea>
                @error('deskripsi')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <x-select id="pengurus_id" name="pengurus_id">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('pengurus_id') == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </x-select>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-border mt-6">
                ...
                <x-button type="submit" variant="default" class="shadow-primary/20 shadow-lg hover:shadow-primary/40">Simpan Wilayah</x-button>
            </div>
        </form>
    </x-card>
</div>
