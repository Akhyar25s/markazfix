...
<div class="space-y-6 animate-fade-in">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-foreground">Manajemen Wilayah</h1>
            <p class="text-muted-foreground mt-1 text-sm sm:text-base">Kelola data wilayah untuk Sistem I'tikaf Markaz.</p>
        </div>
    ...
    <x-card class="overflow-hidden border-primary/10 shadow-xl relative backdrop-blur-md bg-card/80">
        ...
        <div class="relative z-10">
            <x-table :headers="['No', 'Nama Wilayah', 'Pengurus', 'Status', 'Aksi']">
                @forelse($wilayahs as $wilayah)
                    <tr class="border-b border-border transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        <td class="p-4 align-middle text-muted-foreground">{{ $loop->iteration + $wilayahs->firstItem() - 1 }}</td>
                        <td class="p-4 align-middle font-medium text-foreground">
                            {{ $wilayah->nama_wilayah }}
                            @if($wilayah->deskripsi)
                                <span class="block text-xs text-muted-foreground font-normal mt-0.5 line-clamp-1">{{ $wilayah->deskripsi }}</span>
                            @endif
                        </td>
                        <td class="p-4 align-middle text-muted-foreground">
                            @if($wilayah->pengurus)
                                <div class="flex items-center gap-2">
                                    ...
                                    <span>{{ $wilayah->pengurus->name }}</span>
                                </div>
                            @else
                                <span class="text-xs italic text-muted-foreground/60">Belum ada pengurus</span>
                            @endif
                        </td>
                        ...
                        <td class="p-4 align-middle">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('wilayah.show', $wilayah->id) }}" class="p-2 text-blue-500 hover:bg-blue-500/10 rounded-md transition-colors" title="Detail">
                                ...
                                <a href="{{ route('wilayah.edit', $wilayah->id) }}" class="p-2 text-accent hover:bg-accent/10 rounded-md transition-colors" title="Edit">
                                ...
                                <form action="{{ route('wilayah.destroy', $wilayah->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus wilayah ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="p-8 text-center text-muted-foreground">
                            <div class="flex flex-col items-center justify-center">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="h-12 w-12 text-muted-foreground/50 mb-3"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path></svg>
